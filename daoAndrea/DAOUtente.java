...
public class DAOUtente {
	...
	public static Utente readUtente(TransactionManager tm, String email)throws DAOException {
    	tm.assertInTransaction();
    	Utente utente = null;
    	try (PreparedStatement ps = tm.getConnection().prepareStatement("SELECT * FROM utenti WHERE email  =?")){
	...
	public static ArrayList<Auto> readAutoUtente(TransactionManager tm, Utente u)throws DAOException{
		...
		tm.assertInTransaction();
		 ...
		 try (PreparedStatement preparedStatement = tm.getConnection().prepareStatement("SELECT * FROM Proprieta WHERE utente="+u.getId())){
