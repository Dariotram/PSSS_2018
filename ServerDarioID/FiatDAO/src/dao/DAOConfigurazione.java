...
public class DAOConfigurazione {
	...
	public static ArrayList<Integer> readValoriConfigurazione(TransactionManager tm, Configurazione conf) throws DAOException{
		...
		try (PreparedStatement preparedStatement = tm.getConnection().prepareStatement("SELECT * FROM Settaggio WHERE configurazione="+conf.getId())) {
   		 try (ResultSet rs = preparedStatement.executeQuery()) {
   			 while (rs.next() == true){
	    		 ...
	    		 listaValori.add(valoreComponente);

   			 }
   		 }
		}
		catch(SQLException e){
			throw new DAOException("Impossibile leggere i valori dei componenti della configurazione"+conf.getId(),e);
			}
